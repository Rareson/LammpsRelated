...
for i = 1:length(d)
    ...
    if strcmp(currentLegend{1,1}{1,1},'cntC')
        ...
        plot(strain,stress,'--','color',[i/(length(d)/2),0.5,0.5]);
    else
        strain = [0;A.data(:,1)];
        stress = [0;(A.data(:,4)-A.data(1,4))];
...
figure
plot(300:300:2400, [fractureStrain(1,6:8),fractureStrain(1,1:5)],'-o')
hold on
plot(300:300:2400, [fractureStrain(2,6:8),fractureStrain(2,1:5)],'-.')
...
figure
plot(300:300:2400, [fractureStress(1,6:8),fractureStress(1,1:5)],'-o')
hold on
plot(300:300:2400, [fractureStress(2,6:8),fractureStress(2,1:5)],'-.')
legend('Compressive','Tensile')
set(gca,'LineWidth',2,'FontSize',24,'FontWeight','normal','FontName','Times')
set(get(gca,'xlabel'),'String','Temperature (K)','FontSize',32,'FontWeight','bold','FontName','Times')
